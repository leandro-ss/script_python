...
PARAM_BE_AVG_TIME = {"metric-path":"Backends|*|{tierName}|{internalName}|Calls per Minute"}
PARAM_BT_CAL_MIN = {"metric-path":"Business Transaction Performance|Business Transactions|{tierName}|{internalName}|Calls per Minute"}
...
URL_METRIC_DATA = "https://tesla2018073118022510.saas.appdynamics.com/controller/rest/applications/{name}}/metric-data"
...
def collect():
    for app in requests.get(URL_APPLICATION, auth=(USR, PWD)).json():
        for bt in requests.get(URL_APP_BT_LIST.format_map(app), auth=(USR, PWD)).json():
            ...
            response = requests.get(URL_METRIC_DATA.format_map(app), auth=(USR, PWD), params=param)
                ...
                if result['metricValues']:
                    try:
                        for value in result['metricValues']:
                            yield result['metricPath'],result['frequency'], value['value'], value['current'], value['min'], value['max'], value['count'], value['startTimeInMillis']
                    except KeyError:
                        print('Error test:'+result['metricPath'])  
                        pass
